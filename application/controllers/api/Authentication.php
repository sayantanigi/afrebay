...
class Authentication extends CI_Controller {
	...
	public function registration() {
		...
		if(!empty($validate)) {
			$responce = array('status'=> 1,'msg'=>'Sorry this member already exist, try with another email.');
			...
			if(!empty($insert_id)) {
				...
				try {
					...
					$mail->send();
					$responce['status'] = "2";
					$responce['msg'] = "We have sent an activation link to your account to continue with the registration process.";
			        echo json_encode($responce);
				} catch (Exception $e) {
					$responce['status'] = "3";
		            $responce['msg'] = "Your message could not be sent. Please, try again later.";
                    echo json_encode($responce);
				}
			} else {
				$responce['status'] = "4";
				$responce['msg'] = "Something went wrong. Please, try again later.";
				echo json_encode($responce);
			}
		}
	}
            ...
            if($_SESSION['afrebay']['userType'] == '1') {
				...
				if(empty($check_sub)) {
					...
					if(empty($profile_check[0]['firstname']) || empty($profile_check[0]['lastname']) || empty($profile_check[0]['email']) || empty($profile_check[0]['gender']) || empty($profile_check[0]['address']) || empty($profile_check[0]['zip']) || empty($profile_check[0]['short_bio'])) {
                        //redirect('profile');
                        $responce = array('status'=> 4,'msg'=>'Redirecting to profile Page.');
					} else {
                        //redirect('jobbid');
                        $responce = array('status'=> 5,'msg'=>'Redirecting to jobbid Page.');
					}
				}
				...
				if(empty($check_sub)) {
                    ...
                    if(empty($profile_check[0]['companyname']) || empty($profile_check[0]['email']) || empty($profile_check[0]['address']) || empty($profile_check[0]['teamsize'])  || empty($profile_check[0]['short_bio'])) {
                		...
                		//redirect('dashboard');
                        $responce = array('status'=> 5,'msg'=>'Redirecting to dashboard Page.');
                	}
                }
			} else {
				//redirect('login');
                $responce = array('status'=> 6,'msg'=>'Redirecting to dashboard Page.');
			}
        } else {
            $responce = array('status'=> 2,'msg'=>'Invalid Email Address or Password.');
        }
        echo json_encode($responce);
    }
    	...
    	if(!empty($formdata['email'])) {
         	...
         	if(!empty($get_email)) {
				...
				try {
					...
					$mail->Password   = "wj8jeml3eu0z";
					$mail->send();
					$responce = array('status'=> 1,'msg'=>'Please check your inbox. We have sent you an email to reset your password.');
				} catch (Exception $e) {
					$responce = array('status'=> 2,'msg'=>'Something went wrong. Please try again later!');
				}
         	} else {
   				$responce = array('status'=> 3,'msg'=>'Invalid Email Id!');
   			}
			echo json_encode($responce);
		}
	}
    ...
    public function logout() {
        ...
        $responce = array('status'=> 1,'msg'=>'You have logged out.');
		echo json_encode($responce);
	}
}
