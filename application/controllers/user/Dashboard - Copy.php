...
class Dashboard extends CI_Controller {
	...
	function showmessage_listS() {
		echo $userfrom_id = $this->input->post('userfromid');
		echo $user_id = $this->input->post('usertoid');
		echo $post_id = $this->input->post('postid');
		...
		if (!empty($get_data)) {
			foreach ($get_data as $key) {
				...
				if ($key->userfrom_id == $_SESSION['afrebay']['userId'] && $key->userto_id == $user_id && $key->postjob_id == $_POST['post_id']) {
					$sent = '<li class="sent">' . $from_pic . '<p>' . $key->message . '</p><div style="font-size: 10px;">'.$key->created_date.'</li>';
				} else {
					$sent = '';
				}
				if ($key->userto_id == $_SESSION['afrebay']['userId'] && $key->userfrom_id == $user_id && $key->postjob_id == $_POST['post_id']) {
					$reply = '<li class="replies">' . $to_pic . '<p>' . $key->message . '</p><div style="font-size: 10px;">'.$key->created_date.'</li>';
				} else {
					$reply = '';
				}
